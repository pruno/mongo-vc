...
abstract class AbstractTestCase extends TestCase implements ServiceLocatorAwareInterface
{
    ...
    public function getCollectionConfig($index = 0)
    {
        $config = null;
        $c = 0;
        foreach ($this->getConfig()['mongodb_virtual_collections']['collections'] as $className => $alias) {
            if ($index == $c) {
                return array(
                    'className'     =>  $className,
                    'alias'         =>  $alias
    ...
    public function getVirtualCollectionConfig($index = 0)
    {
        $config = null;
        $c = 0;
        foreach ($this->getConfig()['mongodb_virtual_collections']['virtual_collections'] as $className => $alias) {
            if ($index == $c) {
                return array(
                    'className'     =>  $className,
                    'alias'         =>  $alias
    ...
    public function getDriverConfig($index = 0)
    {
        $driver = null;
        $c = 0;
        foreach ($this->getConfig()['mongodb_virtual_collections']['drivers'] as $alias => $driver) {
            if ($index == $c) {
                return array(
                    'alias'     =>  $alias,
                    'driver'    =>  $driver
